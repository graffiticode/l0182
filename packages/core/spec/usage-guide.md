...
L0182 is a survey record. A program names the survey being taken and
— once something has answered — carries the response to it: the options chosen in priority order,
plus one new option that was not in the set. The survey itself is NOT written in the program. Its
options, its title, its instructions and its bounds come from the back end, which the compiler
...
comes back with its options, its title, its instructions and its bounds.
...
| `id`               | The survey                                               |
| :----------------- | :------------------------------------------------------- |
| `civic-priorities` | civic priorities — twelve versions, one drawn per taking |
| `team-retro`       | eight engineering-team retro options                     |
...
numbered — `civic-priorities-1`, `civic-priorities-2`, … — and naming one takes it outright, with no
draw.
...
Name each option by its **exact text**: the options live in the survey, so the ids are not something
...
survey is in front of you. `write-in` is one new option that must not already be in the set; that is
...
every option there is. Choosing everything is not choosing.
...
The survey. There is no word for a set of options, a title, an instruction line or a bound, and
