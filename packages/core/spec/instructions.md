...
L0182 is a **survey record**: the options someone was asked to choose between, and the response to
them — the options chosen in priority order, plus one new option that was not in the set.
...
authored here**: a program cannot write the options, the title, the wording or the bounds, and
...
says nothing else about it — the options come from the survey itself, when the program compiles.
...
person was shown. The options, the title, the instructions and the bounds all come back in the
...
| :----------------- | :---------------------------------------------------------------------------------------------------------- | :------ |
| `civic-priorities` | civic priorities — what people want their representatives to work on. Twelve versions; taking it draws one. | 1–5     |
...
so on. Naming the survey (`id "civic-priorities"`) draws one of them, and which one is recorded as
`instance` in the compiled record. Naming a version outright (`id "civic-priorities-7"`) takes
exactly that one.
...
- **Name each option by its exact text.** This is the important rule. The options live in the survey,
...
- An id in quotes (`choices ["t3"]`) or a position counting from 0 (`choices [2 0]`) is
  accepted **only when you can see the set** — when the compiled survey is in front of you. A
  guessed position lands in range and records the wrong options without failing.
...
- `write-in` is one new option from whoever answered. It must NOT repeat an option already in the set —
...
fails outright when a name matches nothing, and silently records options nobody chose when it
...
- `options` — the set, each with an id. The id is the originating service's own where the survey's
  data carried one, and positional (`o0` upward) where it did not.
...
| `survey`     | `<list: record>`   |   1   | The survey being taken, and optionally the response to it. The options themselves come from the survey, not from the program.                                                                                                                                                                                                                                                                 |
| `response`   | `<list: record>`   |   1   | The options chosen, in priority order, and optionally one new option that was not in the set.                                                                                                                                                                                                                                                                                                 |
...
survey [
  ...
  session-id get-val-public "itemId"
  response [
    choices ["plant shade trees along the bus routes" "extend library hours into the evening"]
    write-in "a late bus home on weekends"
  ]
]..
```

Note what is NOT here: no options, no title, no instructions, no bounds. Writing any of them is a
